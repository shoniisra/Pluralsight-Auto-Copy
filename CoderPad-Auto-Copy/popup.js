// Popup script para manejar la interfaz de usuario - CoderPad Auto Copy
document.addEventListener('DOMContentLoaded', function() {
    ...
    async function handleAutoModeToggle() {
        ...
        try {
            ...
            if (activeTab.url.includes('screen-ide.coderpad.io')) {
    ...
    copyButton.addEventListener('click', async function() {
        ...
        try {
            ...
            // Verificar si estamos en CoderPad
            if (!tab.url.includes('screen-ide.coderpad.io')) {
                showStatus('⚠️ Esta extensión solo funciona en CoderPad', 'error');
                return;
            }
            ...
            if (response && response.success && response.text) {
                ...
                showStatus(response?.message || '❌ No se encontró contenido', 'error');
            }
            
        } catch (error) {
            console.error('Error:', error);
            if (error.message.includes('Receiving end does not exist')) {
                showStatus('❌ Recarga la página de CoderPad e intenta de nuevo', 'error');
            } else {
                showStatus('❌ Error: ' + error.message, 'error');
            }
        } finally {
            copyButton.disabled = false;
            copyButton.textContent = '📋 Copiar Contenido Actual';
        }
    });
    ...
    chrome.tabs.query({ active: true, currentWindow: true }, function(tabs) {
        ...
        if (!currentTab.url.includes('screen-ide.coderpad.io')) {
            showStatus('Navega a screen-ide.coderpad.io para usar la extensión', 'info');
        } else {
            showStatus('💡 Si no funciona, recarga la página de CoderPad primero', 'info');
        }
    });
});
